...
# Restart WinUtil - use VBS to hide PowerShell completely with RunAs
...
objShell.ShellExecute "powershell.exe", "-NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File ```"`$winutilFullPath```"", "", "runas", 0
"@
`$vbsPath = Join-Path `$env:TEMP "start-winutil.vbs"
